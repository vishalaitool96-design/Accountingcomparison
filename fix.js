...
// 1. ADD DIAGNOSTIC BUTTON AND PANEL
...
const diagnosticBlock = 
  '<div class="flex flex-col sm:flex-row justify-between items-start gap-4">' +
    '<div class="border-l-8 border-teal-600 pl-4 py-1 shrink-0">' +
      ...
      '<p class="text-sm text-slate-600">Premium Analysis Guide: AS vs Ind AS 115 vs IFRS 15 vs US GAAP ASC 606</p>' +
    '</div>' +
    '<button id="diagnostic-btn" class="bg-indigo-600 text-white hover:bg-indigo-700 text-sm px-4 py-2 rounded shadow-sm font-bold transition-colors">' +
  ...
  '<div id="diagnostic-panel" class="hidden mt-4 bg-indigo-50 border-l-4 border-indigo-600 p-4 text-sm text-indigo-900 shadow-sm rounded-r-lg flex flex-col gap-2 shrink-0">' +
    ...
    '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">' +
      '<div>' +
        '<h4 class="font-bold border-b border-indigo-200 pb-1 mb-1">Live Updating Sections</h4>' +
        '<ul id="diag-updating" class="list-disc pl-5 space-y-1 text-indigo-800 font-medium">' +
            '<li>Section C (Side-by-Side Treatment)</li>' +
            '<li>Section D (Side-by-Side Impact Table)</li>' +
            '<li>Section E (P&L Impact)</li>' +
            '<li>Section F (Balance Sheet Impact)</li>' +
            '<li>Section G (Cash Flow Impact)</li>' +
            '<li>Section H (Ratio Impact)</li>' +
            '<li>Section I (Earnings Quality Signals)</li>' +
            '<li>Section J (Trend Distortion Matrix)</li>' +
            '<li>Section K (Analyst Concerns)</li>' +
        '</ul>' +
      '</div>' +
      '<div>' +
        '<h4 class="font-bold border-b border-indigo-200 pb-1 mb-1">Static/Hardcoded Sections</h4>' +
        '<ul id="diag-static" class="list-disc pl-5 space-y-1 text-indigo-800 font-medium">' +
            '<li>Section A (Core Technical Comparison)</li>' +
            '<li>Section L (Investor Perception Analysis)</li>' +
            '<li>Section M (Reporting Signals Decoder)</li>' +
            '<li>Section N (Premium Intelligence Narrative)</li>' +
            '<li>Section O (Red/Green Flags Visual Indicator)</li>' +
            '<li>Section P (Final Intelligence Summary)</li>' +
        '</ul>' +
      '</div>' +
    '</div>' +
    '<div class="mt-4 border-t border-indigo-200 pt-2">' +
      '<h4 class="font-bold mb-1 flex justify-between">Recent Updates (Console Log) <span id="diag-errors" class="text-red-600 hidden">Errors Detected!</span></h4>' +
...
html = html.replace(/<section class="([^"]+)">\\n\\s*<div([^>]+>\\n\\s*SECTION [A-Z].*?<\\/div>)/gs, addSectionIds);

// Inject updated JS block
const scriptReplacement = 
  ...
  '      dLog("Ignored Section O, P (marked static).");\\n' +
  '    }\\n' +

...
const calcIdx = html.indexOf('const iRent = document.getElementById');
const calcEndIdx = html.indexOf('              // Copy button');

if (calcIdx === -1 || calcEndIdx === -1) {
...
const newScript = `const iRent = document.getElementById("input-rent");
              ...
              function calc() {
                ...
                try {
                  ...
                  let pv = 0;
                  if (rate > 0) {
                    pv = rent * ((1 - Math.pow(1 + rate, -term)) / rate);
                  } else {
                    pv = rent * term;
                  }
                  ...
                  set("bs-eq-as", "Higher"); // in appearance
                  ...
                  const now = new Date();
                  dLog("Calculated at " + now.toLocaleTimeString());
                } catch (e) {
                  dLog(e.message, "error");
                }
              }
              ...
              if (pvCalcBtn) {
                 pvCalcBtn.addEventListener("click", () => {
                     ...
                     alert("Present Value Calculation:\\n\\n" + formula + "\\n\\nThe ROU Asset and Lease Liability initially equal this PV amount.");
                 });
              }

`;

html = html.substring(0, calcIdx) + newScript + html.substring(calcEndIdx);
fs.writeFileSync('lease.html', html);
console.log('Fixed calculation in lease.html');
